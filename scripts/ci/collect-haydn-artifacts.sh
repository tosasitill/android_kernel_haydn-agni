#!/usr/bin/env bash
# SPDX-License-Identifier: GPL-2.0

set -Eeuo pipefail

log() {
	printf '[haydn-ci] %s\n' "$*"
}

die() {
	printf '[haydn-ci] error: %s\n' "$*" >&2
	exit 1
}

: "${OUT_DIR:=/out}"
: "${ARTIFACT_DIR:?ARTIFACT_DIR is required}"
: "${MODE:=normal}"
: "${TOOLCHAIN:=cpullvm}"
: "${ANYKERNEL_DIR:=anykernel3}"

boot_dir="${OUT_DIR}/arch/arm64/boot"
mkdir -p "${ARTIFACT_DIR}"

copy_if_present() {
	local src="$1"
	local dst="$2"

	if [ -f "${src}" ]; then
		mkdir -p "$(dirname "${dst}")"
		cp "${src}" "${dst}"
		log "collected ${src}"
	fi
}

for image in Image Image.gz Image.gz-dtb Image.lz4 Image.lz4-dtb; do
	copy_if_present "${boot_dir}/${image}" "${ARTIFACT_DIR}/${image}"
done

copy_if_present "${boot_dir}/dtb.img" "${ARTIFACT_DIR}/dtb.img"
copy_if_present "${boot_dir}/dtbo.img" "${ARTIFACT_DIR}/dtbo.img"
copy_if_present "${OUT_DIR}/.config" "${ARTIFACT_DIR}/config"
copy_if_present "${OUT_DIR}/System.map" "${ARTIFACT_DIR}/System.map"
copy_if_present "${OUT_DIR}/defconfig.log" "${ARTIFACT_DIR}/logs/defconfig.log"
copy_if_present "${OUT_DIR}/build.log" "${ARTIFACT_DIR}/logs/build.log"
copy_if_present "${OUT_DIR}/build-metadata.txt" "${ARTIFACT_DIR}/build-metadata.txt"

while IFS= read -r module; do
	rel="${module#${OUT_DIR}/}"
	copy_if_present "${module}" "${ARTIFACT_DIR}/modules/${rel}"
done < <(find "${OUT_DIR}" -type f -name '*.ko' | sort)

if [ -d "${boot_dir}/dts" ]; then
	while IFS= read -r dts_artifact; do
		rel="${dts_artifact#${boot_dir}/}"
		copy_if_present "${dts_artifact}" "${ARTIFACT_DIR}/${rel}"
	done < <(
		find "${boot_dir}/dts" -type f \( \
			-name '*haydn*.dtb' -o \
			-name '*haydn*.dtbo' -o \
			-name '*lahaina*.dtb' -o \
			-name '*lahaina*.dtbo' \
		\) | sort
	)
fi

if ! find "${ARTIFACT_DIR}" -maxdepth 1 -type f -name 'Image*' -print -quit | grep -q .; then
	die "no kernel image was collected from ${boot_dir}"
fi

raw_zip="${ARTIFACT_DIR%/}/../${MODE}-${TOOLCHAIN}-raw.zip"
(
	cd "${ARTIFACT_DIR}"
	zip -qr "${raw_zip}" .
)
log "created raw artifact zip: ${raw_zip}"

if [ -d "${ANYKERNEL_DIR}" ] && [ -f "${ARTIFACT_DIR}/Image" ]; then
	anykernel_stage="${ARTIFACT_DIR%/}/../anykernel-stage"
	rm -rf "${anykernel_stage}"
	mkdir -p "${anykernel_stage}"
	cp -a "${ANYKERNEL_DIR}/." "${anykernel_stage}/"
	cp "${ARTIFACT_DIR}/Image" "${anykernel_stage}/Image"
	copy_if_present "${ARTIFACT_DIR}/dtb.img" "${anykernel_stage}/dtb.img"
	copy_if_present "${ARTIFACT_DIR}/dtbo.img" "${anykernel_stage}/dtbo.img"

	anykernel_zip="${ARTIFACT_DIR%/}/../${MODE}-${TOOLCHAIN}-anykernel.zip"
	(
		cd "${anykernel_stage}"
		zip -qr "${anykernel_zip}" .
	)
	log "created AnyKernel zip: ${anykernel_zip}"
else
	log "skipping AnyKernel zip; ${ANYKERNEL_DIR} or Image is missing"
fi
